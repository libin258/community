...
public class RedisTests {
    ...
    // 统计20万个重复数据的独立总数 : 20万个数据去除重复数据之后的数据
    @Test
    public void testHyperLogLog(){
        ...
        for (int i = 1; i <= 100000 ; i++) {
            ...
            redisTemplate.opsForHyperLogLog().add(redisKey,n);
        }
    ...
    public void testHyperLogUnion(){
        ...
        // union是把数据合并后再存回redis中 类型也是HyperLogLog 重新定义一个key
        String unionKey = "test:hll:union";
        // union中第一个参数为存放合并后的数据的key  后面是要合并数据的key(也可以是数组)
    ...
    public void testBitMap(){
        ...
        // 不是一个独立的数据机构 ,就是一个对string的特殊操作

        // 记录  按位存数据  默认为false 指定它的索引 索引从0开始
        ...
        // 统计   统计的方法不在opsForValue()中
        Object obj = redisTemplate.execute(new RedisCallback() {
            @Override
            public Object doInRedis(RedisConnection connection) throws DataAccessException {

                // 统计Byte里面1的个数
                return connection.bitCount(redisKey.getBytes());  // 返回值会返回给execute方法
            }
        });
